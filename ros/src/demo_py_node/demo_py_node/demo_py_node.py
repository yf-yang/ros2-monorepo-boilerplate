...
class DemoPyNode(Node):
  ...
  def _publish_message(self) -> None:
    ...
    self.get_logger().info(f"published to /py_to_cpp: [#{message.sequence}] '{message.content}'")

  def _on_message(self, message: Greeting) -> None:
    self.get_logger().info(f"received on /cpp_to_py from '{message.sender}': [#{message.sequence}] '{message.content}'")
