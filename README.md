...
我们基于 front matter 构建了文档查阅工具（`scripts/docs.js`），提供两个 pixi 入口。两者都接受 glob pattern 参数（相对于 `docs/` 目录匹配文件路径，多个 pattern 之间为 OR 关系）：

```bash
pixi run list-docs                        # 列出所有文档的元数据摘要
pixi run list-docs 'tooling/*'            # 按 glob 过滤，仅列出匹配的文档元数据
pixi run read-docs '**/quality*'          # 打印匹配文档的完整内容
```
